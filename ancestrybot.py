...
def findAncestor(firstname, lastname):
    # Open a file
    fo = open("foo.txt", "r")
    ...
    for line in fo:
        person = line.split(',')
        if(person[8] == (firstname) and lastname == (person[9])):
            return(person[3]+" "+person[4]+" "+person[5]+" "+person[6]+" "+person[7])
    return("didn't work.")
...
async def on_message(message):
    ...
    elif message.content.startswith('!ancestry'):
        name = message.content[message.content.index(' ') + 1:]
        name = name.split('_')
        firstname = name[0]
        lastname = name[1] 
        msg = findAncestor(firstname, lastname)
